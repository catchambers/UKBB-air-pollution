...
```{r model-2}
res.model2 <- coxph(
  Surv(timeStudy, lung_cancer == 1) ~ pm25_2010per5 + ageBaseline + sex + householdIncomeCat + eduCat + bmiCat + smokingCat,
  data = df_pheno,
  ties = "efron"
)
summary(res.model2)
...
#### SJ Plot
```{r SJ-Model2}
logit_data2 = res.model2 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data2, "master_files/SJ_Plot2")
```
