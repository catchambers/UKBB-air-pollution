...
There are `r sum(lung_cancer)` cases of lung cancer in the data set.
...
```{r model-1-pm25-2010}
res.model1 <- coxph(
  Surv(timeStudy, lung_cancer == 1) ~  pm25_2010per5 + ageBaseline + sex,
  data = df_pheno,
  ties = "efron"
)
summary(res.model1)
...
```{r SJ-Model1}
logit_data = res.model1 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot1")
```
...
logit_data = res_model_2_pm25_2010 %>% sjPlot::get_model_data(type="est")
logit_data
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-pm25-2010")
```
...
logit_data = res_model_2_pm10_2010 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-pm10-2010")
```
...
logit_data = res_model_2_pm10_2007 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot__model_2_pm10_2007")
```

#### PM Coarse (2007)

```{r model-2-pmcoarse-2007}
res_model_2_pmcoarse_2007 <- coxph(
  Surv(timeStudy, lung_cancer) ~ pmcoarse_2010per5 + ageBaseline + sex + householdIncomeCat + eduCat + bmiCat + smokingCat + race,
  data = df_pheno,
  ties = "efron"
)
summary(res_model_2_pmcoarse_2007)

logit_data = res_model_2_pmcoarse_2007 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-pmcoarse-2007")
```
...
logit_data = res_model_2_no2_2005 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-no2-2005")
```
...
logit_data = res_model_2_no2_2006 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-no2-2006")
```
...
logit_data = res_model_2_no2_2007 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-no2-2007")
```
...
logit_data = res_model_2_no2_2010 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-no2-2010")
...
logit_data = res_model_2_no1_2010 %>% sjPlot::get_model_data(type="est")
write_xlsx(logit_data, "master_files/SJ_Plot_model-2-no1-2010")
